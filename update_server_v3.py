#!/usr/bin/env python3
"""
Update the EKS MCP Server with the new SDK V3 implementation
"""
...
def update_server():
    """Update the server with the new SDK V3 implementation"""
    # Define paths
    ...
    print("Updating main.py to use the new SDK V3 implementation")
    with open(main_py, "r") as f:
        content = f.read()
    
    # Add import for the new SDK implementation
    if "from k8s_operations_sdk_v3 import KubernetesOperationsSDKV3" not in content:
    ...
    if "k8s_sdk_v3_ops = KubernetesOperationsSDKV3()" not in content:
        content = content.replace(
    ...
    for handler in handlers:
        if handler in content:
            ...
            new_line = line.replace("k8s_sdk_ops", "k8s_sdk_v3_ops")
